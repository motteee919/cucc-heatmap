...
const sba=supaAdmin();const { data:rows } = await sba.from('track_meta').select('track_id,w,e,s,n').gt('e',w).lt('w',e).gt('n',s).lt('s',n).limit(3000);
if(!rows?.length)return NextResponse.json({type:'FeatureCollection',features:[]});
const files=await Promise.all(rows.map(r=>sba.storage.from('tracks').download(`${r.track_id}-${lod}.json`)));
